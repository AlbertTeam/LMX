...
public class LineHough {
        ...
        public int[] process(int i,int j) {
            w=width/15;//如果width，height未先在init（）中另赋初值，则此处w=0，h=0，需注意赋值先后顺序
            h=height/15;
            ...
            for (int x = i; x < i+ w && x<width; x++) {
                for (int y = j; y < j+ h && y<height; y++) {
                    if (input[y * width + x]  == Color.BLACK) {
                        for (int theta = 0; theta <181; theta=theta+1)
                        { //r=((int)(x * Math.cos(((theta) * Math.PI) / 180) + y * Math.sin(((theta) * Math.PI) / 180))/2)*2;
